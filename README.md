...
- **`search`** — searches messages by text, sender, date range, or topic, and prints a table or exports JSON, CSV, JSON lines, Markdown or HTML. Messages carrying a photo or file are marked `[media]`, and a poll shows its question and answers instead. `search --archive` answers the same flags from the local archive, offline.
