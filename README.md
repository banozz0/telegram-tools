...
`apply` reads the target first and plans only the difference: topics to make, topics and settings to change, in blueprint order, the chat's own settings last. It never plans a delete — a topic or setting the target has beyond the blueprint is reported as *left alone*. The dry-run prints every step; `--execute` asks you to type the target's exact title (with `--create`, the title the new chat will have), re-resolves the chat after you answer and refuses with `PLAN_DRIFT` if it changed, then makes one step at a time. A step Telegram refuses stops the apply there, with everything already made kept and recorded, so a rerun `diff` shows what remains. Afterwards the chat is read back and compared: anything still pending is `partial` (exit 1), and the readback says which. Topic order is never pending: Telegram keeps its own, so a topic the file places after General and Telegram lists before it is not a change, before the apply or after it. Each accepted step appends its own line to `~/.telegram-tools/audit.jsonl`; the remap rows live in the archive under the apply id, and `structure remap` prints them without connecting.
...
Every write shows who acts, on whom, in which chat, and what changes, then asks. Three of them are `delete`'s gate, because they take something away from a person rather than adding to a chat: `member ban`, `member kick` and `admin demote` dry-run by default, take `--execute`, ask you to type the person's exact label — the prompt names the one every screen around it printed, `Harry (@harry)`, and takes the `@username` alone as well — refuse without a terminal in either mode, and have no `--yes`. Everything else asks `y/N`, and on all of it but `settings set` `--yes` answers the question: the preview still prints, nothing else changes, and no allowlist applies — a `--yes` here is a script or an agent saying the user already asked for exactly this.
...
Telegram widens what a restriction names, and the previews say so, because the preview is the screen your `y` answers. `member mute` names `send_messages` and Telegram takes the fifteen rights beneath it — every kind of sending, embedded links included; `member restrict --rights send_media` takes the six media kinds as well as `send_media` itself; a right with nothing beneath it, like `send_stickers`, takes only itself. `member ban` is the widest: it takes every right there is, `view_messages` among them, so the person cannot even read the chat, and it has no end — Telegram stores a ban as the 32-bit time ceiling, which the screens say as `permanently` rather than as a date in 2038. `admin promote` and `admin rights` are the same shape upwards: Telegram sets `other` beside whatever rights you name, the flag that carries an admin's baseline (the admin log, chat statistics, the member list, ignoring slow mode), so the preview names it too. What is sent is still exactly what you asked for; the widening is Telegram's, and now the preview and the readback name the same rights.

Invite links are credentials to a chat, so they appear only where you asked for them: `invite list` and `invite create` print them and carry them in `result`. Every other screen, envelope field and audit line goes through the same redaction pass as a token, which blanks a link — including the one you handed to `invite revoke`, except in that command's human preview, which shows it back to you because its y/N is about that link.
