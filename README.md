...
- **`search`** — searches messages by text, sender, date range, or topic, and prints a table or exports JSON, CSV, JSON lines, Markdown or HTML. Messages carrying a photo or file are marked `[media]`, a poll shows its question and answers instead, and a forwarded message is marked `[fwd …]` with where it came from — which a copy, having dropped the author on purpose, never is. `search --archive` answers the same flags from the local archive, offline.
