...
Checked on 2026-09-06 for the administration commands (Telethon 1.44.0):

- `channels.GetParticipantRequest(channel, participant)` returns `channels.ChannelParticipant` with one `participant` and its `users`; `InputUserSelf()` asks for the caller, which works for the account and for a bot. Someone not in the chat raises `UserNotParticipantError`. `channels.GetParticipantsRequest(channel, filter, offset, limit, hash=0)` pages at 200; the filters used are `ChannelParticipantsAdmins`, `ChannelParticipantsRecent`, `ChannelParticipantsSearch(q)`, `ChannelParticipantsKicked(q)` (banned, `view_messages` on) and `ChannelParticipantsBanned(q)` (restricted).
- The participant shapes: `ChannelParticipantCreator(user_id, admin_rights, rank)`, `ChannelParticipantAdmin(user_id, promoted_by, admin_rights, can_edit, rank)`, `ChannelParticipant(user_id, date)`, `ChannelParticipantSelf`, and `ChannelParticipantBanned(peer, kicked_by, banned_rights, left)` — the banned one carries a `peer`, not a `user_id`, and `left` says whether they are gone or merely restricted. `can_edit` is Telegram's own answer to "may the caller edit this admin"; an admin promoted by someone else reads `False`.
- `channels.EditAdminRequest(channel, user_id, admin_rights, rank)` sets an admin's rights to exactly the set given; all-off demotes. `channels.EditBannedRequest(channel, participant, banned_rights)` is one call for a ban (`view_messages=True`), a restriction (send rights on, `until_date` set) and lifting either (nothing on, no date). `until_date` more than 366 days out means forever to Telegram, which is why `--until` is bounded there.
- `telethon.tl.custom.ParticipantPermissions` (what `get_permissions` returns) spells `add_admins` beside `ban_users`, `invite_users`, `change_info` and the rest, so the preflight vocabulary can name it. It spells neither `manage_topics` nor the send rights, so those stay unknown to the probe and a plan needing one goes through with the warning rather than a refusal.
- Join requests are `messages.GetChatInviteImportersRequest(peer, offset_date=None, offset_user=InputUserEmpty(), limit, requested=True)`, whose `importers` are `ChatInviteImporter(user_id, date, requested, about)`; `messages.HideChatJoinRequestRequest(peer, user_id, approved)` answers one either way.
- Invite links: `messages.GetExportedChatInvitesRequest(peer, admin_id=InputUserSelf(), limit, revoked=None|True)` lists the caller's links (an admin sees only its own, the creator every admin's through `admin_id`); `messages.ExportChatInviteRequest(peer, title=, expire_date=, usage_limit=, request_needed=)` returns the new `ChatInviteExported` directly; `messages.EditExportedChatInviteRequest(peer, link, revoked=True)` returns `messages.ExportedChatInvite` whose `invite` is the revoked link. There is no `--reason` on any of these, and there is no reason field on a ban: the local audit line is the record.
- A broadcast channel has no slow mode (`ToggleSlowModeRequest` refuses), and every call above is a `channels.*` call: a basic group has none of them until Telegram upgrades it to a supergroup, which the app does on the first settings change.
