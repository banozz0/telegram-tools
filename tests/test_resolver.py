...
def test_resolve_numeric_chat_id_from_dialogs_before_get_entity():
    ...
    client = FakeClient(
        dialogs=[
            ...
            SimpleNamespace(id=-1003930298580, entity=entity, input_entity=input_entity),
        ]
    )

    resolved = asyncio.run(resolve_chat(client, "-1003930298580"))

    assert resolved.id == -1003930298580
